...
DeviceSpecs getDeviceSpecs() {
    ...
    specs.maxFreq = std::stoi(
            FSUtil::readFile("/sys/devices/system/cpu/cpu7/cpufreq/cpuinfo_max_freq"));
    return specs;
}
...
void cpuLittleTweak() {
    // Adjust the path to only target CPUs 0 to 5
    std::vector <std::string> paths;
    for (int i = 0; i < 6; ++i) { // CPU 0 to CPU 5
        paths.push_back("/sys/devices/system/cpu/cpu" + std::to_string(i) + "/cpufreq");
    }
    ...
    Logger(LogType::INFO) << "Completed CPU_LITTLE_CLUSTER tweak" << std::endl;
}

void cpuBigTweak() {
    // Adjust the path to target CPUs 6 and 7
    std::vector <std::string> paths;
    for (int i = 6; i <= 7; ++i) { // CPU 6 to CPU 7
        paths.push_back("/sys/devices/system/cpu/cpu" + std::to_string(i) + "/cpufreq");
    }
    ...
    Logger(LogType::INFO) << "Completed CPU_BIG_CLUSTER tweak" << std::endl;
}
...
void ioTweak() {
    ...
    for (const std::string &path: paths) {
        ...
        std::vector <std::string> preferredScheds = {"kyber", "bfq", "cfq", "mq-deadline", "noop",
                                                     "none"};
...
void allTweaks() {
    Logger(LogType::INFO) << "[Start] Userspace Optimizer Tweaks" << std::endl;
    ...
    Logger(LogType::INFO) << "[End] Userspace Optimizer Tweaks" << std::endl;
}
